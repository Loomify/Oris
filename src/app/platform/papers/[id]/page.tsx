...
export default async function Paper(args: any) {
    ...
    if (token != undefined) {
        ...
        let journal_info = await db.select().from(journal).where(eq(journal.id, papers[0].journal_id))
        if (journal_info.length == 0 && papers[0].journal_id != null) {
            await db.update(paper).set({journal_id: 0}).where(eq(paper.id, args.params.id))
        }
            ...
            <div className="container">
                ...
                <div className="platform_body">
                    ...
                    <main className='platform_content_paper'>
                        <Link href={(papers[0]['journal_id'] == null || papers[0]['journal_id'] == 0) ? '#' : `/platform/journals/${papers[0].journal_id}`} className='paper_left' style={{backgroundImage: `url(${((await db.select().from(journal).where(eq(journal.id, papers[0].journal_id))).length != 0) ? (await db.select().from(journal).where(eq(journal.id, papers[0].journal_id)))[0].image_url : ("/not_in_a_journal.png")})`}}>
                        </Link>
        ...
        return (
            <div className="container">
                ...
                <div className="platform_body">
                    <PlatformSidebar />
                    <main className='platform_content_paper'>
                        <Link href={(papers[0]['journal_id'] == null) ? '#' : `/platform/journals/${papers[0].journal_id}`} className='paper_left' style={{backgroundImage: `url(${((await db.select().from(journal).where(eq(journal.id, papers[0].journal_id))).length != 0) ? (await db.select().from(journal).where(eq(journal.id, papers[0].journal_id)))[0].image_url : ("/not_in_a_journal.png")})`}}>
                        </Link>
                        <div className='paper_meta'>
                            <h1>{papers[0]['title']}</h1>
                            <p>By {papers[0]['authors']}</p>
